  ...
  try {
    ...
    try {
      ...
      if (quotaErr.name === 'QuotaExceededError' || quotaErr.message.includes('quota')) {
        console.warn('[requests] Quota exceeded. Attempting to clear and save reduced list.');
        // First, definitely remove the old one to free space
        await AsyncStorage.removeItem(REQUESTS_KEY);

        // Try saving only the most recent ones
        if (Array.isArray(payload) && payload.length > 0) {
          const reduced = payload.slice(0, 5);
          try {
            await AsyncStorage.setItem(REQUESTS_KEY, JSON.stringify(reduced));
          } catch (e2) {
            console.warn('[requests] Even reduced list failed. Leaving storage empty for this key.');
          }
        }
      } else {
        throw quotaErr;
