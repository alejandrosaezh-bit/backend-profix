...
export default function ManageJobModal({ visible, onClose, job, onUpdate }) {
    ...
    const handlePickImage = async () => {
        const result = await ImagePicker.launchImageLibraryAsync({
            ...
            allowsEditing: false,
            aspect: [4, 3],
            quality: 1, // High quality initial, we resize next
        ...
        if (!result.canceled) {
            setLoading(true);
            try {
                // Upload each selected image after resizing
                for (const asset of result.assets) {
                    ...
                    await onUpdate('add-photo', { url: manipResult.uri });
                }
            } catch (error) {
                console.log(error);
                Alert.alert('Error', 'No se pudo subir alguna de las fotos.');
            } finally {
                setLoading(false);
            }
        }
