...
export default function ChatScreen({ request, currentUser, userMode, onBack, onSend, onViewJob }) {
  ...
  const messages = useMemo(() => {
    return (currentConversation && Array.isArray(currentConversation.messages)) ? currentConversation.messages : [];
  }, [currentConversation]);
    ...
    const handleReceiveMessage = (data) => {
      ...
      if (data.chatId === currentConversation.id) {
        setLocalMessages(prev => {
          ...
          // If it's from me, it should theoretically be ignored if we have a robust optimistic UI,
          // but we'll add it if it's not already there (the 'exists' check above handles this).
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        const fullChat = await api.getChatDetails(currentConversation.id);
        if (fullChat && fullChat.messages) {
          const formatted = fullChat.messages.map(m => ({
            id: m._id,
            text: m.content,
            sender: areIdsEqual(m.sender?._id || m.sender, currentUser?._id || currentUser?.id) ? (isActingAsPro ? 'pro' : 'client') : (isActingAsPro ? 'client' : 'pro'),
            timestamp: m.createdAt || m.timestamp,
            media: m.media,
            mediaType: m.mediaType
          }));

          // Solo actualizar si el historial es más largo o contiene datos nuevos
          // para evitar parpadeos si ya lo teníamos todo.
          setLocalMessages(formatted);
        }
      } catch (e) {
        console.warn("[ChatScreen] Error fetching history:", e);
      }
    };
    fetchHistory();
  }, [currentConversation?.id]);
