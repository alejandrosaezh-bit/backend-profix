        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
            <View style={[styles.avatarContainer, {
              ...
              {(() => {
                // FAST LOOKUP (O(1)) instead of find (O(N))
                const catName = typeof item.category === 'object' ? item.category.name : item.category;
                const subName = typeof item.subcategory === 'object' ? item.subcategory.name : item.subcategory;

                // 1. Try to find full category object in our Map
                const fullCat = categoryMap[catName] || (typeof item.category === 'object' ? item.category : null);

                // 2. Try Subcategory
                // If we mapped subcategories, we could look them up. 
                // For now, let's look for icon in fullCat

                let iconKey = null;

                // Check subcategory override inside fullCat
