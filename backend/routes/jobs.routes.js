...
router.post('/:id/offers', protect, async (req, res) => {
    try {
        ...
        if (io) {
            // Notify the client specifically
            io.to(`user_${job.client}`).emit('notification', {
                type: 'new_offer',
                title: 'Nueva Oferta',
                body: `${req.user.name} ha enviado un presupuesto.`,
                jobId: job._id
            });
            // Also refresh job details if they are viewing it
            io.emit(`job_${job._id}_updated`, job);
        }
...
router.put('/:id/assign', protect, async (req, res) => {
    try {
        ...
        const io = req.app.get('socketio');
        if (io) {
            io.to(`user_${professionalId}`).emit('notification', {
                type: 'offer_accepted',
                title: '¡Ganaste el trabajo!',
                body: `Tu oferta para ${job.title} ha sido aceptada.`,
                jobId: job._id
            });
            io.emit(`job_${job._id}_updated`, job);
        }
    ...
    try {
        ...
        if (io) {
            io.to(`user_${job.client}`).emit('notification', {
                type: 'work_started',
                title: 'Trabajo Iniciado',
                body: `El profesional ha comenzado la labor.`,
                jobId: job._id
            });
            io.emit(`job_${job._id}_updated`, job);
        }
...
router.get('/me', protect, async (req, res) => {
    try {
        ...
        const jobsWithChats = await Promise.all(jobs.map(async job => {
            ...
            const conversations = jobChats.map(chat => {
                const clientVal = job.client;
                const jobClientIdStr = clientVal ? ((clientVal._id || clientVal).toString()) : '';
                const proParticipant = chat.participants.find(p => (p._id || p).toString() !== jobClientIdStr);
        ...
        try {
            const fs = require('fs');
            fs.appendFileSync('backend_error.log', `[${new Date().toISOString()}] GET /jobs/me Error: ${error.stack}\n`);
        } catch (e) {
            console.error("Error writing backend log:", e);
        }
...
router.delete('/:id', protect, async (req, res) => {
    try {
        ...
        console.log(`[DELETE JOB] Deleting reviews for job ${job._id}...`);
        try {
            const reviewResult = await Review.deleteMany({ job: job._id });
            console.log(`[DELETE JOB] Deleted ${reviewResult.deletedCount} reviews.`);
        } catch (err) {
            console.error(`[DELETE JOB] Error deleting reviews:`, err);
        }
